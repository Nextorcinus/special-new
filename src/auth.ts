...
function getDiscordAvatarUrl(
	...
	const defaultAvatar = Number(
		(BigInt(discordId) / BigInt(2) ** BigInt(22)) % BigInt(6),
	);
...
export const { auth, handlers, signIn, signOut } = NextAuth({
	...
	providers: [
		Discord({
			...
			profile(profile) {
				const discordProfile = getDiscordProfile(profile);

				if (!discordProfile.id) {
					console.error("[Auth] Discord profile ID is missing.");
				...
				const image = getDiscordAvatarUrl(discordId, discordProfile.avatar);
				...
				console.log("[Auth] Discord profile received:", {
					discordId,
					username,
				});
	...
	callbacks: {
		async signIn({ user }) {
			try {
				...
				if (!discordId) {
					console.error("[Auth] Discord user ID is missing during signIn.");
				...
				const avatar = typeof user.image === "string" ? user.image : null;

				console.log("[Auth] Discord login:", {
					discordId,
					username,
				});
				...
				console.log("[Auth] User database sync successful:", {
		...
		async session({ session, token }) {
			if (session.user) {
				...
				session.user.name =
					typeof token.username === "string" ? token.username : "Discord User";
