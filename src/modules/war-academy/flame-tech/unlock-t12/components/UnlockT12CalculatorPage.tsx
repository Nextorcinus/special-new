...
function isUnlockT12History(history: UnlockT12HistoryItem): boolean {
	return history.module === "unlock-t12";
}
...
function normalizeHistoryEntries(
	...
	if (Array.isArray(history.items) && history.items.length > 0) {
		return history.items as UnlockT12HistoryEntry[];
	}
...
export default function UnlockT12CalculatorPage(
	...
	return (
		...
		>
			<UnlockT12CalculatorPageContent {...props} />
		</Suspense>
	);
}
...
function UnlockT12CalculatorPageContent({
	...
	useEffect(() => {
		...
		if (
			!isUnlockT12History(selectedHistory) ||
	...
	function handleCompleted(entryId: string) {
		...
		setActiveHistory(updated.history as UnlockT12HistoryItem);
	}
