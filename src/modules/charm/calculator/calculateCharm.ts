...
function getLevelIndex(
	...
	return data.findIndex(
		(item) => getLevelValue(item.level) === selectedLevel,
	);
}
...
function calculateResources(
	...
	return rows.reduce<CharmResourceMap>(
		(total, item) => {
			total.Guide += getNumericValue(
				item,
				"guide_cost",
			);

			total.Design += getNumericValue(
				item,
				"design_cost",
			);

			total.Jewel += getNumericValue(
				item,
				"jewel_cost",
			);
...
function calculateSvsPoints(
	...
	return rows.reduce(
		(total, item) =>
			total + getNumericValue(item, "svs_point"),
		0,
	);
}
...
function calculateStats(
	...
	const powerFrom = getNumericValue(
		fromRow,
		"power_total",
	);

	const powerToRaw = getNumericValue(
		toRow,
		"power_total",
	);

	const statFrom = getNumericValue(
		fromRow,
		"stat_total",
	);

	const statToRaw = getNumericValue(
		toRow,
		"stat_total",
	);
	...
	const powerIncrease = upgradeRows.reduce(
		(total, item) =>
			total + getNumericValue(item, "power_diff"),
		0,
	);

	const statIncrease = upgradeRows.reduce(
		(total, item) =>
			total + getNumericValue(item, "stat_diff"),
		0,
	);
	...
	const powerTo =
		powerToRaw > 0
			? powerToRaw
			: powerFrom + powerIncrease;

	const statTo =
		statToRaw > 0
			? statToRaw
			: statFrom + statIncrease;
...
function validateForm(
	...
	if (!values.type) {
		throw new Error(
			"Pilih tipe Chief Charm terlebih dahulu.",
		);
	}
...
export function calculateCharm(
	...
	if (!Array.isArray(data) || data.length === 0) {
		throw new Error(
			"Data Chief Charm tidak tersedia.",
		);
	}

	const sortedData = [...data].sort(
		(firstItem, secondItem) =>
			getLevelValue(firstItem.level) -
			getLevelValue(secondItem.level),
	);
	...
	const baseSvsPoints =
		calculateSvsPoints(upgradeRows);

	const parsedValeriaLevel = Number.parseInt(
		values.valeriaLevel,
		10,
	);

	const valeriaLevel = Number.isFinite(
		parsedValeriaLevel,
	)
		? Math.min(
				Math.max(parsedValeriaLevel, 0),
				10,
			)
		: 0;
	...
	const valeriaBonusPoints =
		baseSvsPoints * (valeriaBonus / 100);

	const svsPoints =
		baseSvsPoints + valeriaBonusPoints;
