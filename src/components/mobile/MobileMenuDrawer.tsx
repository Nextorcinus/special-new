...
type NavigationGroupId = "chief" | "heroes" | "development";
...
const ADDITIONAL_ITEMS = ["troops", "war-academy", "widget", "state"];
...
function getGroupItems(group: NavigationGroupId) {
	return NAVIGATION.filter((item) => item.group === group);
}

function groupHasActiveItem(pathname: string, group: NavigationGroupId) {
	return getGroupItems(group).some((item) => isPathActive(pathname, item.href));
}
...
export default function MobileMenuDrawer({
	...
	useEffect(() => {
		...
		const activeGroup = GROUPS.find((group) =>
			groupHasActiveItem(pathname, group.id),
		);

		setOpenGroups({
			chief: activeGroup?.id === "chief",
			heroes: activeGroup?.id === "heroes",
			development: activeGroup?.id === "development",
		});
	}, [open, pathname]);
	...
	function toggleGroup(group: NavigationGroupId) {
		setOpenGroups((current) => {
			const isCurrentlyOpen = current[group];
				...
				<div className="min-h-0 flex-1 overflow-y-auto px-3 py-4">
					<div className="space-y-2">
						{GROUPS.map((group) => {
							...
							const hasActive = groupHasActiveItem(pathname, group.id);
				...
				<div className="shrink-0 space-y-3 border-t border-[var(--sl-border)] bg-[var(--sl-surface)] p-4">
					...
					<div className="px-1 text-center">
						<p className="text-[9px] leading-4 text-[var(--sl-text-muted)]">
							© 2026 Special Lazyness. Not affiliated with Century Games or
							Whiteout Survival.
						</p>
					</div>
				</div>
